...
func LambdaHandler(cxt context.Context, event events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	log.Logger.Debug().Msg("Start lambda update animal")
	...
	log.Logger.Debug().Msgf("animal %v", animalPayload)
	err = inj.Instances["animalService"].(serviceanimal.AnimalService).Update(animalPayload)
	if err != nil {
		log.Logger.Error().Err(err).Msgf("ERROR on the animal %v", animalPayload)
		return util.ResponseErrorFunction(err, fmt.Sprintf("Error when it is process request")), err
	}
